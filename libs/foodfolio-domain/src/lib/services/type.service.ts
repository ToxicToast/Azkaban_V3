...
export class TypeService {
	...
	async getTypeById(id: string): Promise<Result<TypeAnemic>> {
		try {
			...
			return Result.fail<TypeAnemic>(GenericErrorCodes.NOT_FOUND);
		} catch (error) {
			return Result.fail<TypeAnemic>(error);
		}
	}

	async getTypeByTitle(title: string): Promise<Result<TypeAnemic>> {
		try {
	...
	async createType(data: TypeData): Promise<Result<TypeAnemic>> {
		try {
			const check = await this.getTypeByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<TypeAnemic>(GenericErrorCodes.UNKNOWN);
			}

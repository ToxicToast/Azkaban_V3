...
export class LocationService {
	...
	async getLocationByTitle(title: string): Promise<Result<LocationAnemic>> {
		try {
	...
	async createLocation(data: LocationData): Promise<Result<LocationAnemic>> {
		try {
			const check = await this.getLocationByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<LocationAnemic>(GenericErrorCodes.UNKNOWN);
			}

...
export class WarehouseService {
	...
	async getWarehouseById(id: string): Promise<Result<WarehouseAnemic>> {
		try {
			...
			return Result.fail<WarehouseAnemic>(GenericErrorCodes.NOT_FOUND);
		} catch (error) {
			return Result.fail<WarehouseAnemic>(error);
		}
	}

	async getWarehouseByTitle(title: string): Promise<Result<WarehouseAnemic>> {
		try {
	...
	async createWarehouse(
		...
		try {
			const check = await this.getWarehouseByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<WarehouseAnemic>(GenericErrorCodes.UNKNOWN);
			}

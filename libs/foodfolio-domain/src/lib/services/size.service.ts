...
export class SizeService {
	...
	async getSizeById(id: string): Promise<Result<SizeAnemic>> {
		try {
			...
			return Result.fail<SizeAnemic>(GenericErrorCodes.NOT_FOUND);
		} catch (error) {
			return Result.fail<SizeAnemic>(error);
		}
	}

	async getSizeByTitle(title: string): Promise<Result<SizeAnemic>> {
		try {
	...
	async createSize(data: SizeData): Promise<Result<SizeAnemic>> {
		try {
			const check = await this.getSizeByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<SizeAnemic>(GenericErrorCodes.UNKNOWN);
			}

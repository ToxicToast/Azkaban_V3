...
export class CategoryService {
	...
	async getCategoryByTitle(title: string): Promise<Result<CategoryAnemic>> {
		try {
	...
	async createCategory(data: CategoryData): Promise<Result<CategoryAnemic>> {
		try {
			const check = await this.getCategoryByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<CategoryAnemic>(GenericErrorCodes.UNKNOWN);
			}

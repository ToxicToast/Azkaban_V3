...
export class CompanyService {
	...
	async getCompanyByTitle(title: string): Promise<Result<CompanyAnemic>> {
		try {
	...
	async createCompany(data: CompanyData): Promise<Result<CompanyAnemic>> {
		try {
			const check = await this.getCompanyByTitle(data.title);
			if (check.isSuccess) {
				return Result.fail<CompanyAnemic>(GenericErrorCodes.UNKNOWN);
			}
